...
export default function EditJobPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (user && isRecruiter) {
      http.get<any>(`/jobs/${resolvedParams.id}`)
        .then((data) => {
          if (data) {
            setTitle(data.title || '');
            setDepartment(data.department || '');
            setLocation(data.location || '');
            setType(data.jobType || 'FULL_TIME');
            setExperienceLevel(data.experienceLevel || 'Senior');
            setSalaryMin(String(data.minSalary || '120000'));
            setSalaryMax(String(data.maxSalary || '160000'));
            setDescription(data.description || '');
            if (Array.isArray(data.requiredSkills)) {
              setSkills(data.requiredSkills.map((s: any) => typeof s === 'string' ? s : s.skillName));
            }
          }
        })
        .catch((err) => {
  ...
  }, [user, authLoading, isRecruiter, router, resolvedParams.id]);

  if (authLoading || loading || (user && !isRecruiter)) {
    return (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await http.put(`/jobs/${resolvedParams.id}`, {
        title,
        department,
        location,
        jobType: type,
      ...
      if (apiErr?.status === 403) {
        ...
        toast.error(apiErr?.message || 'Failed to update job requisition.');
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="space-y-6 pt-2 max-w-4xl mx-auto">
      ...
      <Card className="glass border-white/10 p-6">
        <CardHeader className="p-0 pb-6 border-b border-white/10">
          <CardTitle className="text-2xl font-bold">Edit Job Requisition</CardTitle>
          <CardDescription className="text-xs">
            Update role requirements, required skills, or target compensation.
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
              ...
              <Button type="submit" disabled={submitting} className="primary-button text-xs gap-2">
                {submitting ? 'Updating...' : 'Update Job Requisition'}
              </Button>
            </div>
          </form>
        </CardContent>
